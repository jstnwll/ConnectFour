...
class ConnectFour:
    ...
    def draw_board(self):
        ...
        if self.phase == PHASE_GAME_OVER:
            ...
            # Display Metrics for AI modes
            if self.game_mode != 1:
                avg_time = 0.0
                if self.ai_move_count > 0:
                    avg_time = self.ai_total_time / self.ai_move_count
                
                metrics_text = f"Moves: {self.ai_move_count} | Avg AI Time: {avg_time:.4f}s"
                m_surf = self.small_font.render(metrics_text, True, SUBTEXT)
                self.screen.blit(m_surf, (status_bar.x + 24, status_bar.y + 40))
    ...
    def update(self):
        ...
        if self.phase == PHASE_ANIMATING:
            ...
            if pygame.time.get_ticks() >= self.ai_timer:
                ...
                if col != -1:
                    self.ai_total_time += (end_time - start_time)
    ...
    def _smart_move(self):
        ...
        # Priority 3: Block Major Threats (Open-ended 2-in-a-row)
        for c in range(BOARD_WIDTH):
            if self.is_valid_move(c):
                if self._blocks_open_ended_threat(c, 1):
                     return c

        # Priority 4: Create Major Threat (Open-ended 3-in-a-row)
        for c in range(BOARD_WIDTH):
            if self.is_valid_move(c):
                # Check if we can create a major threat for ourselves (player 2)
                if self._blocks_open_ended_threat(c, 2) and not self._creates_opponent_threat(c):
                    return c

        # Priority 5: Create threat (2 in a row with potential for 3), but ensure safety
        threat_cols = []
        for c in range(BOARD_WIDTH):
            if self.is_valid_move(c):
        ...
        # Priority 6: Avoid giving opponent easy wins, prefer center
        safe_cols = []
        for c in range(BOARD_WIDTH):
            if self.is_valid_move(c):
        ...
        # Fallback: prefer center among valid moves
        valid = [c for c in range(BOARD_WIDTH) if self.is_valid_move(c)]
    ...
    def _blocks_open_ended_threat(self, col, opponent):
        """Check if placing a piece at col blocks an open-ended 2-in-a-row for opponent."""
        row = self.get_next_open_row(self.board, col)
        if row is None: return False
        
        # Vertical Check: Opponent has 2 pieces below current row
        if row <= BOARD_HEIGHT - 3:
            if self.board[row+1][col] == opponent and self.board[row+2][col] == opponent:
                return True

        # Horizontal Check
        # Left side check: Opp at col-1, col-2, and Empty at col-3
        if col >= 3:
            if (self.board[row][col-1] == opponent and 
                self.board[row][col-2] == opponent and 
                self.board[row][col-3] == 0):
                return True
        # Right side check: Opp at col+1, col+2, and Empty at col+3
        ...
        # Gap Check: X _ X (Opp at col-1 and col+1)
        if col >= 1 and col <= BOARD_WIDTH - 2:
            if self.board[row][col-1] == opponent and self.board[row][col+1] == opponent:
                return True
